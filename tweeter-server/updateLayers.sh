# For each Lambda function, give it the layer / dependencies
...
do
    ...
    # Add --region $REGION to the aws lambda update-function-configuration command
    aws lambda update-function-configuration --function-name "$function_name" --layer "$LAMBDALAYER_ARN" --region $REGION 1>>/dev/null & 
